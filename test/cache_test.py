...
class RedistCacheTest(unittest.TestCase):
    ...
    def generate_samples(self):
        strs = [self.get_random_string(20) for i in range(10000)]
        ...
        strxs = [self.get_random_string(21) for i in range(1000)]
    ...
    def test_cache(self, cache: AdtCache):
        strs, samples = self.generate_samples()
    ...
    def test_memcache(self):
        ...
        for i in range(epochs):
            timings = self.test_cache(MemoryCache())
    ...
    def test_rediscache(self):
        ...
        epoch_list = []
        for i in range(epochs):
            timings = self.test_cache(RedisCache(host="localhost", port=6379, db=1))
            values = list(timings.values())
            deltas = [values[i + 1] - values[i] for i in range(len(values) - 1)]
            print(deltas)
